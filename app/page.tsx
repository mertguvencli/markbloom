            ...
            <div className="flex items-center gap-2">
              ...
              <Popover open={exportOpen} onOpenChange={setExportOpen}>
                ...
                <PopoverContent align="end" className="w-56 p-3">
                  ...
                  <div className="mb-3 grid grid-cols-3 gap-1 rounded-md bg-zinc-100 p-1">
                    {Object.entries(pageSizeLabels).map(([value, label]) => (
                      <button
                        key={value}
                        type="button"
                        onClick={() => setPageSize(value as PageSize)}
                        className={
                          pageSize === value
                            ? "rounded bg-white px-2 py-1 text-xs font-medium text-zinc-900 shadow-sm"
                            : "rounded px-2 py-1 text-xs text-zinc-500 hover:text-zinc-900"
                        }
                      >
                        {label}
                      </button>
          ...
          <div ref={containerRef} className="flex flex-1 flex-col overflow-hidden md:flex-row">
            ...
            >
              <div className="flex-1 overflow-hidden">
                <MarkdownEditor value={markdown} onChange={updateContent} />
              </div>
            </div>
            ...
            {/* Preview panel */}
            <div className="min-h-[200px] flex-1 overflow-auto md:min-h-0 md:min-w-[280px]">
              <MarkdownPreview
                ref={previewRef}
                content={markdown}
                pageSize={pageSize}
